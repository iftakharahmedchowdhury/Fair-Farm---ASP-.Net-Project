...
using BLL.DTOs.TraderDTOs;

namespace BLL.Services.Trader
{ 

     public class TraderEqupmentRentService
{
    public static EquipmentRentDTO Add(EquipmentRentDTO s)
    {
        ...
        var addedData = DataAccessFactoryTrader.TraderEquipmentRentData().Add(data);
    ...
    {
        var data = DataAccessFactoryTrader.TraderEquipmentRentData().Get(id);
    ...
    {
        var data = DataAccessFactoryTrader.TraderEquipmentRentData().GetByRegion(id);
    ...
    {
        var data = DataAccessFactoryTrader.TraderEquipmentRentData().GetmyOwnRequest(id);
        if (data == null)
        {
    ...
    {
        var data = DataAccessFactoryTrader.TraderEquipmentRentData().GetRentRequests(ownerid);
        if (data != null)
        {
            var config = new MapperConfiguration(cfg =>
            {
    ...
    public static EquipmentRentDTO UpdateforMyRentRequest(int renteruserid, int equipmentid)
    {
        ...
        var checkequpment = DataAccessFactoryTrader.TraderEquipmentRentData().Get(equipmentid);
        ...
        var updatedData = DataAccessFactoryTrader.TraderEquipmentRentData().UpdateforRentReq(equipmentid, renteruserid);
        var config2 = new MapperConfiguration(cfg =>
        {
            cfg.CreateMap<EquipmentRent, EquipmentRentDTO>();
        });
    ...
    public static EquipmentRentDTO UpdatebyEqupmentOwner(int id, int ownerid, EquipmentRentDTO s)
    {
        ...
        var checkequpment = DataAccessFactoryTrader.TraderEquipmentRentData().Get(id);
        ...
        var updatedData = DataAccessFactoryTrader.TraderEquipmentRentData().Update(data);
        var config2 = new MapperConfiguration(cfg =>
        {
            cfg.CreateMap<EquipmentRent, EquipmentRentDTO>();
        });
    ...
    public static bool Delete(int id, int ownerid)
    {
        ...
        return DataAccessFactoryTrader.TraderEquipmentRentData().Delete(id);
    }

    public static TraderEquipmentRentDTO GetRentRelatedHistory(int id)
    {
        ...
        {
            c.CreateMap<User, TraderEquipmentRentDTO>();
            c.CreateMap<EquipmentRent, EquipmentRentDTO>();

        });
        var mapper = new Mapper(cfg);
        var mapped = mapper.Map<TraderEquipmentRentDTO>(data);
        return mapped;
    }
}
